...
"""Entry point for win rate data collection (two models, same env seeds).

Collects trajectories from two policies on the same env seeds and saves as a
TrajectoryDataset.  Win rate computation is done offline (visualizer, AI
annotation, or RM scoring).

Usage::

    bash examples/embodiment/collect_winrate.sh maniskill_collect_winrate
"""
...
@hydra.main(
    version_base="1.1",
    config_path="config",
    config_name="maniskill_collect_winrate",
)
def main(cfg) -> None:
